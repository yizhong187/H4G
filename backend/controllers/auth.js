...
import { insertUser, findUser, insertToken, removeToken } from "../models/auth.js";
...
export async function registration (req, res) {
    try {
        ...
        if (existingUser.rows.length !== 0) {
            throw new CustomError(400, "This email is linked to an existing account");
        }
...
export async function login (req, res) {
    try {
        ...
        const jwtUser = { id: user.rows[0].id };
        ...
        await insertToken(user.rows[0].id, hashedToken); // stores the refresh token in the database
